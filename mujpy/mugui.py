...
##########################
    def fft(self):
        ...
        def on_fft_request(b):
            ...
            '''
            import numpy as np
            from mujpy.auxiliary.auxiliary import derange, derange_int, autops, ps, _ps_acme_score, _ps_peak_minima_score, plotile, get_title
...
#                    if self._single_:
        ...
        def on_range(change):
            ...
            '''
            from mujpy.auxiliary.auxiliary import derange
...
# with common (global) and run dependent (local) parameters
    ...
    # [i for i in range(len(name)) if name.startswith('_', i)]
        ...
        def fitplot(guess=False,plot=False):
            ...
            '''
            import matplotlib.pyplot as P
            from mujpy.auxiliary.auxiliary import derange_int, rebin, get_title, plotile, set_bar
            ...
            def animate(i):
                ...
                '''
                # from mujpy.auxiliary.auxiliary import get_title
                # print('animate')
        ...
        def int2_int():
            ...
            '''
            from mujpy.auxiliary.auxiliary import translate
...
##########################################
        ...
        def on_fit_request(b):
            ...
            from mujpy.auxiliary.auxiliary import derange_int, rebin, norun_msg, get_title
    ...
    ###################
        ...
        def on_function_changed(change):
            ...
            check for validity of function syntax
            '''
            from mujpy.auxiliary.auxiliary import muvalid
        ...
        def on_group_changed(change):
            ...
            '''
            from mujpy.auxiliary.auxiliary import get_grouping
    ...
    #       now parse groupcsv shorthand
        ...
        def on_range(change):
            ...
            check for validity of function syntax
            '''
            from mujpy.auxiliary.auxiliary import derange_int
        ...
        def set_group():
            ...
            for k, name in enumerate(groups):
                s = ''
                auxiliary = np.split(self.grouping[name],np.where(np.diff(self.grouping[name]) != 1)[0]+1)
                for j in auxiliary:
...
################
    def plots(self):
        ...
        def on_counterplot(b):
            ...
            from mujpy.auxiliary.auxiliary import get_grouping, norun_msg, derange_int
            import matplotlib.pyplot as P
        ...
        def on_multiplot(b):
            ...
            import matplotlib.pyplot as P
            from numpy import array
            from mujpy.auxiliary.auxiliary import derange_int, rebin, get_title
            import matplotlib.animation as animation
        ...
        def on_range(change):
            ...
            check for validity of function syntax
            on_range (PLOTS, FIT, FFT) perhaps made universal and moved to auxiliary
            '''
            from mujpy.auxiliary.auxiliary import derange
        ...
        def on_tlogdisplay(b):
            ...
            import os
            import matplotlib.pyplot as P
            from mujpy.auxiliary.auxiliary import muzeropad
...
##########################
    def setup(self):
        ...
        def promptfit(mplot = False, mprint = False):
            ...
            from mujpy.auxiliary.auxiliary import norun_msg
...
#                       if self.first_t0plot:
        ...
        def save_log(b):
            ...
            from mujpy.auxiliary.auxiliary import value_error
...
##########################
    def suite(self):
        ...
        def run_headers(k):
            ...
            '''
            import numpy as np
            from mujpy.auxiliary.auxiliary import get_title, value_error
            if k==0:
                try:
        ...
        def add_runs(k,runs):
            ...
            by means of murs2py.
            runs is a list of strings containing integer run numbers (provided by auxiliary.derange)
        ...
        def on_loads_changed(change):
            ...
            '''
            from mujpy.auxiliary.auxiliary import derun, tlog_exists
